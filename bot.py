...
def handle_query(call):
    ...
    if call.data == "account":
        ...
        if last_bonus and (now - last_bonus).total_seconds() < 86400:
            ...
            bot.answer_callback_query(call.id, text=f"⏳ يمكنك المحاولة بعد {hours} ساعة و {minutes} دقيقة.")
        else:
            users_data[user_id]["balance"] += 0.00000050
            users_data[user_id]["last_bonus"] = now
            bot.answer_callback_query(call.id)
            bot.send_message(call.message.chat.id, "🎁 تم إضافة 0.00000050 BTC إلى رصيدك كمكافأة يومية.")
        ...
        if balance >= 0.0001:
            ...
            bot.answer_callback_query(call.id)
            bot.send_message(call.message.chat.id, f"✅ تم إرسال طلب سحب {balance:.8f} BTC إلى {email}.")
        ...
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id, "📢 سيتم عرض الإعلانات هنا قريباً.")
